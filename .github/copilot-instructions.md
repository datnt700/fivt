...
- Tech Stack: Next.js 15 (App Router, React 19), TypeScript, Prisma (PostgreSQL), NextAuth (magic link + Google), next-intl for i18n, Vitest (unit), Playwright (e2e), Tailwind CSS v4, Radix UI primitives, React Query for data fetching / caching.
...
- i18n: next-intl middleware (`src/middleware.ts`) auto localizes all non-excluded routes. Avoid manual locale prefixes in redirects; use relative paths so middleware re-writes. Exclusions: API routes, protected dashboard/banking routes, static assets.
...
- Unit test files may live under `src/**` with `.test.` or `.spec.` suffix or in `tests/` (see `turbo.json` inputs). When adding new tests, follow those globs to ensure cache invalidation.
...
4. Write unit test in same folder (`*.test.ts(x)`) or under `tests/` referencing the action.
5. Run `./scripts/dev.ps1 all` before commit.
