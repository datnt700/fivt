...
function switchSideToClasses(side: Side) {
  switch (side) {
    case 'top':
      return 'data-[state=closed]:slide-out-to-top data-[state=open]:slide-in-from-top inset-x-0 top-0 h-auto border-b';
    case 'right':
      return 'bg-background data-[state=open]:animate-in data-[state=closed]:animate-out fixed z-50 flex flex-col gap-4 shadow-lg transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500';
    case 'bottom':
  ...
  return (
    <SheetPortal>
      ...
      <SheetPrimitive.Content
        ...
        className={cn(switchSideToClasses(side), className)}
        {...props}
      >
        {children}
